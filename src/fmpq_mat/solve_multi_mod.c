                        ...
                        const fmpz_mat_t A, const fmpz_mat_t B)
{
    ...
    {
        flint_throw(FLINT_ERROR, "Exception (fmpq_mat_solve_fmpz_mat_multi_mod). Incompatible matrix dimensions.\n");
    }

    fmpz_mat_init(Z, B->r, B->c);
    fmpz_init(den);

    success = fmpz_mat_solve_multi_mod_den(Z, den, A, B);
    if (success)
...
fmpq_mat_solve_multi_mod(fmpq_mat_t X, const fmpq_mat_t A, const fmpq_mat_t B)
{
    ...
    if (A->r != B->r || A->c != X->r || X->c != B->c)
    {
        flint_throw(FLINT_ERROR, "Exception (fmpq_mat_solve_multi_mod). Incompatible matrix dimensions.\n");
    }
